...
class ThreadActiveRecord extends ActiveRecord
{
    ...
    {
        return [
            ['name', 'required', 'message' => t( 'Topic can not be blank.')],
    ...
    public function requiredPollAnswers()
    {
        if ($this->pollAdded) {
            ...
            if (count($this->pollAnswers) < 2) {
                $this->addError('pollAnswers', t( 'You have to add at least 2 options.'));
            }
        }
    }
    ...
    {
        return [
            'pollAuestion' => t( 'Question'),
            'pollVotes' => t( 'Number of votes'),
            'pollHidden' => t( 'Hide results before voting'),
            'pollEnd' => t( 'Poll ends at'),
        ];
    }
