...
use app\modules\bizley\podium\src\db\UserQuery;
...
use app\modules\bizley\podium\src\models\Mod;
...
abstract class UserActiveRecord extends ActiveRecord implements IdentityInterface
{
    ...
    public function passwordRequirements($attribute)
    {
        if (!preg_match('~\p{Lu}~', $this->$attribute) ||
            ...
            $this->addError($attribute, t( 'Password must contain uppercase and lowercase letter, digit, and be at least 6 characters long.'));
        }
    }
    ...
    public function validateCurrentPassword($attribute)
    {
        if (!$this->hasErrors()) {
            if (!$this->validatePassword($this->currentPassword)) {
                $this->addError($attribute, t( 'Current password is incorrect.'));
            }
        }
    }
    ...
    public function validateUsername($attribute)
    {
        if (!$this->hasErrors()) {
            if (!preg_match('/^[\p{L}][\w\p{L}]{2,254}$/u', $this->username)) {
                $this->addError($attribute, t( 'Username must start with a letter, contain only letters, digits and underscores, and be at least 3 characters long.'));
            }
        }
    }
}
