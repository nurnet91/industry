...
use app\modules\bizley\podium\src\models\db\MessageActiveRecord;
...
class Message extends MessageActiveRecord
{
    ...
    public function podiumReport($post = null)
    {
        try {
            ...
            $this->topic = t( 'Complaint about the post #{id}', ['id' => $post->id]);
            if (Podium::getInstance()->podiumConfig->get('use_wysiwyg') == '0') {
                $this->content .= "\n\n---\n"
                            . '[' . t( 'Direct link to this post') . '](' . Url::to(['forum/show', 'id' => $post->id]) . ')'
                            . "\n\n---\n"
                            . '**' . t( 'Post contents') . '**'
                            . $post->content;
            } else {
                $this->content .= '<hr>'
                            . Html::a(t( 'Direct link to this post'), ['forum/show', 'id' => $post->id])
                            . '<hr>'
                            . '<p>' . t( 'Post contents') . '</p>'
