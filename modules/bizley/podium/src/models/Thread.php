...
use app\modules\bizley\podium\src\models\db\ThreadActiveRecord;
...
class Thread extends ThreadActiveRecord
{
    ...
    {
        $description = t( 'No New Posts');
        $append = false;

        if ($this->locked) {
            $description = t( 'Locked Thread');
            $append = true;
        } elseif ($this->pinned) {
            $description = t( 'Pinned Thread');
            $append = true;
        } elseif ($this->posts >= Podium::getInstance()->podiumConfig->get('hot_minimum')) {
            $description = t( 'Hot Thread');
            $append = true;
        }
        if ($this->userView) {
            if ($this->new_post_at > $this->userView->new_last_seen) {
                if (!$append) {
                    $description = t( 'New Posts');
                } else {
                    $description .= ' (' . t( 'New Posts') . ')';
                }
            } elseif ($this->edited_post_at > $this->userView->edited_last_seen) {
                if (!$append) {
                    $description = t( 'Edited Posts');
                } else {
                    $description = ' (' . t( 'Edited Posts') . ')';
                }
            }
