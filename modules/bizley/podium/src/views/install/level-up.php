...
<?php if ($error == '' && $info == ''): ?>
        <div class="form-group">
            <em><?= t( 'Podium will attempt to update all database tables to the current version.') ?></em><br>
            <em><strong class="text-danger"><?= t( 'Back up your existing database and then click the button below.') ?></strong></em>
        </div>
        ...
        <div class="form-group">
            <button id="installPodium" class="btn btn-warning btn-lg"><span class="glyphicon glyphicon-open"></span> <?= t( 'Upgrade Podium Database') ?></button>
        </div>
<?php elseif ($error != ''): ?>
...
<?php endif; ?>
        <div class="form-group">
            <?= t( 'Current database version') ?> <kbd><?= $dbVersion ?></kbd> <span class="glyphicon glyphicon-transfer"></span> <kbd><?= $currentVersion ?></kbd> <?= t( 'Current module version') ?>
        </div>
<?php if ($info == ''): ?>
...
<div class="row hide" id="installationResults">
    ...
    <div class="col-sm-8 col-sm-offset-2 hide" id="installationError">
        <div class="alert alert-danger" role="alert"><?= t( 'There was a major error during update...') ?></div>
    </div>
    <div class="row hide" id="installationFinished">
        <div class="text-center col-sm-12">
            <a href="<?= Url::to(['forum/index']) ?>" class="btn btn-success btn-lg"><span class="glyphicon glyphicon-ok-sign"></span> <?= t( 'Update finished') ?></a>
        </div>
    </div>
    <div class="row hide" id="installationFinishedError">
        <div class="text-center col-sm-12">
            <button class="btn btn-danger btn-lg"><span class="glyphicon glyphicon-alert"></span> <?= t( 'Errors during update') ?></button>
        </div>
    </div><br>
