...
$this->params['breadcrumbs'][] = ['label' => t( 'Podium Installation'), 'url' => ['install/run']];
...
<div class="row" id="startInstallation">
    <div class="text-center col-sm-12">
        <div class="form-group">
            <em><?= t( 'Podium will attempt to create all database tables required by the forum along with the default configuration and the administrator account.') ?></em><br>
            <em><strong><?= t( 'Back up your existing database and then click the button below.') ?></strong></em>
        </div>
        <div class="alert alert-warning">
            <span class="glyphicon glyphicon-exclamation-sign"></span> <?= t( 'Seriously - back up your existing database first!') ?><br>
            <?= t( 'Podium does its best to make sure your data is not corrupted but make a database copy just in case.') ?><br>
            <?= t( 'You have been warned!') ?>*
        </div>
        <div class="alert alert-danger">
            <label><input type="checkbox" name="drop" value="1" id="drop"> <?= t( 'Check this box to drop all existing Podium tables first') ?> <span class="glyphicon glyphicon-alert"></span></label><br>
            <?= t( '(all existing Podium data will be deleted!)') ?>
        </div>
        <div class="form-group">
            <button id="installPodium" class="btn btn-success btn-lg"><span class="glyphicon glyphicon-import"></span> <?= t( 'Start Podium Installation') ?></button>
        </div>
        <div class="form-group">
            <?= t( 'Version to install') ?> <kbd><?= $version ?></kbd>
        </div>
        <div class="form-group text-muted">
            <small>* <?= t( 'Podium cannot be held liable for any database damages that may result directly or indirectly from the installing process. Back up your data first!') ?></small>
        </div>
    </div>
</div>
<div class="row hide" id="installationResults">
    ...
    <div class="col-sm-8 col-sm-offset-2 hide" id="installationError">
        <div class="alert alert-danger" role="alert"><?= t( 'There was a major error during installation! Check your runtime log for details.') ?></div>
    </div>
    <div class="row hide" id="installationFinished">
        <div class="text-center col-sm-12">
            <a href="<?= Url::to(['forum/index']) ?>" class="btn btn-success btn-lg"><span class="glyphicon glyphicon-ok-sign"></span> <?= t( 'Installation finished') ?></a>
        </div>
    </div>
    <div class="row hide" id="installationFinishedError">
        <div class="text-center col-sm-12">
            <button class="btn btn-danger btn-lg"><span class="glyphicon glyphicon-alert"></span> <?= t( 'Errors during installation') ?></button>
        </div>
    </div><br>
