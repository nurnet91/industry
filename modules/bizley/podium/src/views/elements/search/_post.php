...
<?php if ($model->postData->edited && $model->postData->edited_at): ?>
                    <em>(<?= t( 'Edited') ?> <span data-toggle="tooltip" data-placement="top" title="<?= Podium::getInstance()->formatter->asDatetime($model->postData->edited_at, 'long') ?>"><?= Podium::getInstance()->formatter->asRelativeTime($model->postData->edited_at) ?>)</span></em>
<?php endif; ?>
                </small>
                <?= $model->postData->author->podiumTag ?>
                <small>
                    <span class="label label-info" data-toggle="tooltip" data-placement="top" title="<?= t( 'Number of posts') ?>"><?= $model->postData->author->postsCount ?></span>
                </small>
            </div>
            <div class="popover-content podium-content">
                ...
                <div class="podium-action-bar">
                    <a href="<?= Url::to(['forum/show', 'id' => $model->postData->id]) ?>" class="btn btn-default btn-xs" data-pjax="0" data-toggle="tooltip" data-placement="top" title="<?= t( 'Direct link to this post') ?>"><span class="glyphicon glyphicon-link"></span></a>
                </div>
            </div>
        </div>
    </div>
