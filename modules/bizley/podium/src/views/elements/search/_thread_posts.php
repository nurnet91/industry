...
<td class="podium-thread-line">
    <a href="<?= Url::to(['forum/show', 'id' => $postModel->id]) ?>" class="podium-go-to-new pull-right" style="margin-right:10px" data-toggle="popover" data-container="body" data-placement="left" data-trigger="hover focus" data-html="true" data-content="<small><?= Html::encode(strip_tags($postModel->parsedContent)) ?><br><strong><?= $postModel->author->podiumName ?></strong> <?= Podium::getInstance()->formatter->asRelativeTime($postModel->updated_at) ?></small>" title="<?= t( 'Found Post') ?>">
