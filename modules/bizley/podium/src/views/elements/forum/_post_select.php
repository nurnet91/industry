...
<div class="row podium-post" id="post<?= $model->id ?>">
    <div class="col-sm-2" id="postAvatar<?= $model->id ?>">
        <?= Html::checkbox('post[]', false, ['value' => $model->id, 'label' => t( 'Select this post')]) ?>
    </div>
...
<?php if ($model->edited && $model->edited_at): ?>
                    <em>(<?= t( 'Edited') ?> <span data-toggle="tooltip" data-placement="top" title="<?= Podium::getInstance()->formatter->asDatetime($model->edited_at, 'long') ?>"><?= Podium::getInstance()->formatter->asRelativeTime($model->edited_at) ?>)</span></em>
<?php endif; ?>
                </small>
                <?= $model->author->podiumTag ?>
                <small>
                    <span class="label label-info" data-toggle="tooltip" data-placement="top" title="<?= t( 'Number of posts') ?>"><?= $model->author->postsCount ?></span>
                </small>
            </div>
