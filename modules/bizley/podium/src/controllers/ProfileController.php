...
use app\modules\bizley\podium\src\models\Email;
use app\modules\bizley\podium\src\models\Meta;
use app\modules\bizley\podium\src\models\Subscription;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->validate()) {
                if ($model->gravatar && empty($user->email)) {
                    $model->addError('gravatar', t( 'You need email address set to use Gravatar.'));
                } else {
                    if ($model->image) {
                        $folderExists = true;
                        if (!file_exists($path)) {
    ...
    public function actionAdd($id = null)
    {
        ...
        $data = [
            ...
            'msg' => Html::tag('span',
                ...
                . ' ' . t( 'Error while adding this subscription!'),
                ['class' => 'text-danger']
            ),
        ];
        ...
        if (Podium::getInstance()->user->isGuest) {
            $data['msg'] = Html::tag('span',
                Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                . ' ' . t( 'Please sign in to subscribe to this thread'),
                ['class' => 'text-info']
            );
        }
        ...
        if (is_numeric($id) && $id > 0) {
            ...
            if (!empty($subscription)) {
                $data['msg'] = Html::tag('span',
                    Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                    . ' ' . t( 'You are already subscribed to this thread.'),
                    ['class' => 'text-info']
                );
            } else {
                if (Subscription::add((int)$id)) {
                    $data = [
                        ...
                        'msg'   => Html::tag('span',
                            ...
                            . ' ' . t( 'You have subscribed to this thread!'),
                            ['class' => 'text-success']
                        ),
                    ];
                }
