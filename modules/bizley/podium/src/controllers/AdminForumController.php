...
use app\modules\bizley\podium\src\models\forms\ConfigForm;
use app\modules\bizley\podium\src\models\Forum;
use app\modules\bizley\podium\src\models\LogSearch;
use app\modules\bizley\podium\src\models\Post;
use app\modules\bizley\podium\src\models\User;
use app\modules\bizley\podium\src\PodiumCache;
...
class AdminForumController extends BaseController
{
    ...
    public function actionSortCategory()
    {
        ...
        if (!is_numeric($modelId) || !is_numeric($new)) {
            return Html::tag('span',
                Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                . ' ' . t( 'Sorry! Sorting parameters are wrong.'),
                ['class' => 'text-danger']
            );
        }
        ...
        if (empty($moved)) {
            return Html::tag('span',
                Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                . ' ' . t( 'Sorry! We can not find Category with this ID.'),
                ['class' => 'text-danger']
            );
        }
        if ($moved->newOrder((int)$new)) {
            return Html::tag('span',
                ...
                . ' ' . t( "New categories' order has been saved."),
                ['class' => 'text-success']
            );
        }
        return Html::tag('span',
            Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
            . ' ' . t( "Sorry! We can not save new categories' order."),
            ['class' => 'text-danger']
        );
    }
    ...
    public function actionSortForum()
    {
        ...
        if (empty($moved) || empty($modelCategory) || $moved->category_id != $movedCategory->id) {
            return Html::tag('span',
                Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                . ' ' . t( 'Sorry! We can not find Forum with this ID.'),
                ['class' => 'text-danger']
            );
        }
        if ($moved->newOrder((int)$new)) {
            return Html::tag('span',
                Html::tag('span', '', ['class' => 'glyphicon glyphicon-ok-circle'])
                . ' ' . t( "New forums' order has been saved."),
                ['class' => 'text-success']
            );
        }
        return Html::tag('span',
            Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
            . ' ' . t( "Sorry! We can not save new forums' order."),
            ['class' => 'text-danger']
        );
    }
}
