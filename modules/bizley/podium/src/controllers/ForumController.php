...
namespace app\modules\bizley\podium\src\controllers;


use app\modules\bizley\podium\src\db\Query;
use app\modules\bizley\podium\src\filters\AccessControl;
use app\modules\bizley\podium\src\models\Category;
use app\modules\bizley\podium\src\models\forms\SearchForm;
...
use app\modules\bizley\podium\src\models\Vocabulary;
use app\modules\bizley\podium\src\rbac\Rbac;
use app\modules\bizley\podium\src\services\ThreadVerifier;
use Exception;
use Yii;
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'ruleConfig' => ['class' => 'app\modules\bizley\podium\src\filters\LoginRequiredRule'],
                'rules' => [
                    ['class' => 'app\modules\bizley\podium\src\filters\InstallRule'],
                    [
                    ...
                    [
                        'class' => 'app\modules\bizley\podium\src\filters\PodiumRoleRule',
                        'allow' => true
                    ],
                ],
            ],
    ...
    public function actionPoll()
    {
        ...
        $data = [
            ...
            'msg' => Html::tag('span',
                ...
                . ' ' . t( 'Error while voting in this poll!'),
                ['class' => 'text-danger']
            ),
        ];
        ...
        if ($this->module->user->isGuest) {
            $data['msg'] = Html::tag('span',
                Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                . ' ' . t( 'Please sign in to vote in this poll'),
                ['class' => 'text-info']
            );
        ...
        if (is_numeric($pollId) && $pollId > 0 && !empty($votes)) {
            ...
            if ($poll->getUserVoted($loggedId)) {
                $data['msg'] = Html::tag('span',
                    ...
                    . ' ' . t( 'You have already voted in this poll.'),
                    ['class' => 'text-info']
                );
                return Json::encode($data);
            }
            ...
            if (empty($checkedAnswers)) {
                $data['msg'] = Html::tag('span',
                    Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                    . ' ' . t( 'You need to select at least one answer.'),
                    ['class' => 'text-warning']
                );
                return Json::encode($data);
            }
            if (count($checkedAnswers) > $poll->votes) {
                $data['msg'] = Html::tag('span',
                    Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                    . ' ' . t( 'This poll allows maximum {n, plural, =1{# answer} other{# answers}}.', ['n' => $poll->votes]),
                    ['class' => 'text-danger']
                );
                return Json::encode($data);
            }
