...
use app\modules\bizley\podium\src\models\MessageReceiver;
use app\modules\bizley\podium\src\models\MessageSearch;
...
class MessagesController extends BaseController
{
    ...
    public function actions()
    {
        return [
            'delete-received' => [
                'class' => 'app\modules\bizley\podium\src\actions\MessageAction',
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                ...
                if (empty($model->receiversId)) {
                    $model->addError('receiversId', t( 'You have to select at least one message receiver.'));
                    $errors = true;
                } else {
                    foreach ($model->receiversId as $r) {
                        if ($r == $podiumUser->id) {
                            $model->addError('receiversId', t( 'You can not send message to yourself.'));
                            $errors = true;
                        } elseif ($podiumUser->isIgnoredBy($r)) {
                            $model->addError('receiversId', t( 'One of the selected members ignores you and has been removed from message receivers.'));
                            $errors = true;
                        } else {
                            ...
                            if ($member) {
                                ...
                                if (count($validated) > Message::MAX_RECEIVERS) {
                                    $model->addError('receiversId', t( 'You can send message up to a maximum of 10 receivers at once.'));
                                    $errors = true;
                                    break;
                                }
                            }
