    ...
    public function actionThumb()
    {
        ...
        $data = [
            ...
            'msg' => Html::tag('span',
                ...
                . ' ' . t( 'Error while voting on this post!'),
                ['class' => 'text-danger']
            ),
        ];
        ...
        if ($this->module->user->isGuest) {
            $data['msg'] = Html::tag('span',
                Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                . ' ' . t( 'Please sign in to vote on this post'),
                ['class' => 'text-info']
            );
        ...
        if (is_numeric($postId) && $postId > 0 && in_array($thumb, ['up', 'down'])) {
            ...
            if ($post) {
                ...
                if ($post->author_id == User::loggedId()) {
                    $data['msg'] = Html::tag('span',
                        Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                        . ' ' . t( 'You can not vote on your own post!'),
                        ['class' => 'text-info']
                    );
                    return Json::encode($data);
                }
                ...
                if ($votes !== false) {
                    if ($votes['expire'] < time()) {
                        ...
                        $data['msg'] = Html::tag('span',
                            Html::tag('span', '', ['class' => 'glyphicon glyphicon-warning-sign'])
                            . ' ' . t( '{max} votes per hour limit reached!', ['max' => 10]),
                            ['class' => 'text-danger']
                        );
                        return Json::encode($data);
                    } else {
