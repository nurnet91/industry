...
$options = [];
for ($a = 1; $a <= 10; $a++) {
    $opts = ['placeholder' => t( 'Leave empty to remove')];
...
<div class="row <?= $index > 2 ? 'podium-poll-opt-' . $index : '' ?> <?= $option['value'] === null && $index > 2 ? 'hide' : '' ?>">
    <?= $form->field($model, $pollAnswers .'[]', $fieldLayoutLong)
            ->label(t( 'Option #{n}', ['n' => $index]), ['for' => $option['id']])
            ->textInput($option); ?>
</div>
<?php endforeach; ?>
<div class="row podium-poll-plus">
    <div class="col-sm-offset-3 col-sm-9">
        <button class="btn btn-default btn-xs"><span class="glyphicon glyphicon-plus"></span> <?= t( 'One more'); ?></button>
    </div>
</div>