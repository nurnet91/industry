...
use app\modules\bizley\podium\src\models\Poll;
...
<?php if (!empty($model->end_at)): ?>
                <small class="pull-right">
                    <span data-toggle="tooltip" data-placement="top" title="<?= Podium::getInstance()->formatter->asDatetime($model->end_at, 'long') ?>">
                        <?= t( 'Poll ends') ?>: <?= Podium::getInstance()->formatter->asRelativeTime($model->end_at) ?>
                    </span>
                </small>
<?php endif; ?>
                <?= t( 'Poll') ?>: <strong><?= Html::encode($model->question) ?></strong>
            </div>
            <div class="popover-content podium-content">
<?php if (!$voted): ?>
                <?= Html::beginForm('#', 'post', ['class' => 'podium-poll-form']) ?>
                    <?= Html::hiddenInput('poll_id', $model->id) ?>
                    <p class="small"><?= t( 'Select {n, plural, =1{only # answer} other{max # answers}}', ['n' => $model->votes]) ?>:</p>
...
<?php endforeach; ?>
                    <ul class="list-inline">
                        <li><button class="btn btn-primary podium-poll-vote"><span class="glyphicon glyphicon-ok-sign"></span> <?= t( 'Vote') ?></button></li>
<?php if (!$hidden): ?>
                        <li><button class="btn btn-default podium-poll-show-results"><?= t( 'See results') ?></button></li>
<?php endif; ?>
...
<?php if ($display === false && ($model->author_id == $loggedId || User::can(Rbac::PERM_UPDATE_POST, ['item' => $model->thread]))): ?>
                    <ul class="podium-action-bar list-inline">
                        <li>
                            <a href="<?= Url::to(['forum/editpoll', 'cid' => $model->thread->category_id, 'fid' => $model->thread->forum_id, 'tid' => $model->thread_id, 'pid' => $model->id]) ?>" class="btn btn-info btn-xs <?= $model->votesCount ? 'disabled text-muted' : '' ?>" data-toggle="tooltip" data-placement="top" title="<?= t( 'Edit Poll') ?>">
                                <span class="glyphicon glyphicon-edit"></span>
                            </a>
                        </li>
                        <li>
                            <a href="<?= Url::to(['forum/deletepoll', 'cid' => $model->thread->category_id, 'fid' => $model->thread->forum_id, 'tid' => $model->thread_id, 'pid' => $model->id]) ?>" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="<?= t( 'Delete Poll') ?>">
...
<?php endif; ?>
<?php if (!$hidden && !$voted): ?>
                    <button class="btn btn-default podium-poll-show-form"><?= t( 'Back to poll') ?></button>
<?php endif; ?>
