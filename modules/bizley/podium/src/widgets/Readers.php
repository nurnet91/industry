...
namespace app\modules\bizley\podium\src\widgets;
...
class Readers extends Widget
{
    ...
    public function run()
    {
        ...
        switch ($this->what) {
            case 'forum':
                $out .= t( 'Browsing this forum') . ': ';
                break;
            case 'topic':
                $out .= t( 'Reading this thread') . ': ';
                break;
            case 'unread':
                $out .= t( 'Browsing unread threads') . ': ';
                break;
            case 'members':
                $out .= t( 'Browsing the members') . ': ';
                break;
        }
        ...
        if ($anonymous) {
            $out .= Html::button(
                t( '{n, plural, =1{# anonymous user} other{# anonymous users}}', [
                    'n' => $anonymous
                ]),
        ...
        if ($guests) {
            $out .= Html::button(
                t( '{n, plural, =1{# guest} other{# guests}}', [
                    'n' => $guests
                ]),
                ['class' => 'btn btn-xs btn-default disabled']
            );
