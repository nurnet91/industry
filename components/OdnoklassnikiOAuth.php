...
class OdnoklassnikiOAuth extends Service
{
    ...
    protected function fetchAttributes()
    {
        ...
        $this->attributes['country'] = $info['location']['country'];
        $this->attributes['city'] = $info['location']['city'];

        $this->attributes['email'] = empty($info['email']) ? $info['uid'].'@'.'ok.reg' : $info['email'];
        $this->attributes['verified_email'] = empty($info['email']) ? 0 : 1;
