 ...
 */
class LinkedinOAuth extends Service
{
    ...
    {
        $info = $this->makeSignedRequest('people/~:(id,first-name,last-name,public-profile-url,email-address,picture-url,location)', [
            'query' => [
                'format' => 'json',
            ],
        ]);
        ...
        $this->attributes['email'] = empty($info['emailAddress']) ? $info['id'].'@'.'linkedin.reg' : $info['emailAddress'];
        $this->attributes['verified_email'] = empty($info['emailAddress']) ? 0 : 1;

        $this->attributes['country'] = $info['location']['name'];

        $this->attributes['photo'] = $info['pictureUrl'];
